...
namespace Mana {
	...
	std::string OpenGLShader::ReadFile(const std::string& filepath)
	{
		...
		if (in)
		{
			...
			{
				MANA_CORE_ERROR("Could not read from file '{0}'", filepath);
			}
		}
		else
		{
			MANA_CORE_ERROR("Could not open file '{0}'", filepath);
		}
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source)
	{
		...
		while (pos != std::string::npos)
		{
			...
			MANA_CORE_ASSERT(Utils::ShaderTypeFromString(type), "Invalid shader type specified");
	...
	void OpenGLShader::CompileOrGetVulkanBinaries(const std::unordered_map<GLenum, std::string>& shaderSources)
	{
		...
		for (auto&& [stage, source] : shaderSources)
		{
			...
			{
				shaderc::SpvCompilationResult module = compiler.CompileGlslToSpv(source, Utils::GLShaderStageToShaderC(stage), m_Path.c_str(), options);
	...
	void OpenGLShader::CompileOrGetOpenGLBinaries()
	{
		...
		for (auto&& [stage, spirv] : m_VulkanSPIRV)
		{
			...
			else
			{
				...
				shaderc::SpvCompilationResult module = compiler.CompileGlslToSpv(source, Utils::GLShaderStageToShaderC(stage), m_Path.c_str());
				if (module.GetCompilationStatus() != shaderc_compilation_status_success)
				{
					MANA_CORE_ERROR(module.GetErrorMessage());
					MANA_CORE_ASSERT(false, "");
	...
	void OpenGLShader::CreateProgram()
	{
		...
		if (isLinked == GL_FALSE)
		{
			...
			MANA_CORE_ERROR("Shader linking failed ({0}):\n{1}", m_Path, infoLog.data());
	...
	void OpenGLShader::Reflect(GLenum stage, const std::vector<uint32_t>& shaderData)
	{
		...
		MANA_CORE_TRACE("OpenGLShader::Reflect - {0} {1}", Utils::GLShaderStageToString(stage), m_Path);
		MANA_CORE_TRACE("    {0} uniform buffers", resources.uniform_buffers.size());
		MANA_CORE_TRACE("    {0} resources", resources.sampled_images.size());

		MANA_CORE_TRACE("Uniform buffers:");
		for (const auto& resource : resources.uniform_buffers)
		{
			...
			MANA_CORE_TRACE("  {0}", resource.name);
			MANA_CORE_TRACE("    Size = {0}", bufferSize);
			MANA_CORE_TRACE("    Binding = {0}", binding);
			MANA_CORE_TRACE("    Members = {0}", memberCount);
		}
	}
}
